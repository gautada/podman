...
RUN echo "%wheel         ALL = (ALL) NOPASSWD: /usr/sbin/sshd,/usr/bin/ssh-keygen,/usr/sbin/crond" >> /etc/sudoers \
 && usermod -aG wheel $USER

USER $USER
WORKDIR /home/bob

RUN podman system connection add local --identity /home/bob/.ssh/podman_key ssh://localhost:22/tmp/podman-run-1000/podman/podman.sock \
 && podman system connection add x86 --identity /home/bob/.ssh/podman_key_x86 ssh://podman-x86.cicd.svc.cluster.local:22/tmp/podman-run-1000/podman/podman.sock \
 && podman system connection add arm --identity /home/bob/.ssh/podman_key_arm ssh://podman-arm.cicd.svc.cluster.local:22/tmp/podman-run-1000/podman/podman.sock
